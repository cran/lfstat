...
streamdefRcmdr <- function(lfobj,
  ...
  table <- match.arg(table, several.ok = FALSE)
  ...
  # MA "pooling" can introduce NAs, do not warn twice
  if(pooling == "MA") {
    x <- suppressWarnings(find_droughts(x, threshold = threshold))
  } else {
  ...
  if(table != "all") {
    table <- c("volmax" = "volume", "durmax" = "duration")[table]
    y$hyear <- water_year(y$start, hyear_start(lfobj))
...
streamdef <- function(lfobj,
  ...
  message("This is a wrapper around the new function 'find_droughts()' including pooling. Please use 'find_droughts()' directly, as in future versions 'streamdef' will be removed.", domain = NA)
